...
我们在`ly-auth-common`中编写一些通用的工具类：
...
- 客户端携带用户名和密码请求登录
- 授权中心调用user-service的接口，根据用户名和密码查询用户信息
- 如果用户名密码正确，能获取用户，否则为空，则登录失败
- 如果校验成功，则生成JWT并写入cookie
...
ly:
  jwt:
    ...
    priKeyPath: D:/heima31/ssh/id_rsa.pri # 私钥地址
    expire: 30 # 过期时间,单位分钟
    cookieName: LY_TOKEN
```
...
public class JwtProperties {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JwtLoginProperties.class);
...
这里的cookie的name，我们配置到属性文件：application.yml：

```yaml
ly:
  jwt:
    secret: ly@Login(Auth}*^37)&heiMa37%f3q2 # 登录校验的密钥
    pubKeyPath: C:/lesson/heima37/rsa/id_rsa.pub # 公钥地址
    priKeyPath: C:/lesson/heima37/rsa/id_rsa.pri # 私钥地址
    expire: 30 # 过期时间,单位分钟
    cookieName: LY_TOKEN
```
...
然后在`JwtProperties`中添加属性：
 ...
 ![1535776580646](assets/1535776580646.png)
...
- 如果cookie被盗用怎么办？
  ...
  - 使用HTTPS协议，防止数据泄漏
- 如果你的微服务地址暴露怎么办？
  - 首先地址不会暴露，因为所有微服务都是通过Zuul进行访问，对外暴露的只有Zuul。
  - 万一暴露了呢？
    - 服务间鉴权。